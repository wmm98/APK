...
if __name__ == '__main__':
    ...
    try:
        ...
        if testApi.getMobileDataEnable() != OSApiConstants.ABLE_TYPE_ENABLE:
            if not commFunc.enableSet(testApi.setMobileDataEnable(OSApiConstants.ABLE_TYPE_ENABLE)):
                logFile.logErr("使能移动上网失败")
            else:
                logFile.logInfo("使能移动上网成功")
        ...
        if current_data != OSApiConstants.ABLE_TYPE_ENABLE:
            logFile.logErr("移动网络使能失败")
        else:
            logFile.logInfo("移动网络使能成功")
