...
if __name__ == '__main__':
    ...
    logFile.logInfo("****关闭状态栏下拉按钮********")
    section = Config.section_system_ui
    iniFile.setSection(section)
    try:
        option = Config.option_statusbar_drop
        if testApi.getStatusbarDrop() != OSApiConstants.ABLE_TYPE_DISABLE:
            if not commFunc.enableSet(testApi.setStatusbarDropEnable(OSApiConstants.ABLE_TYPE_DISABLE)):
                logFile.logErr("无法关闭状态栏下拉按钮")
            logFile.logInfo("成功关闭状态栏下拉按钮")
