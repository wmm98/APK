...
if __name__ == '__main__':
    ...
    logFile.logInfo("****打开满屏按钮********")
    section = Config.section_system_ui
    iniFile.setSection(section)
    try:
        option = Config.option_status_bar
        if testApi.getStatusbarEnable() != OSApiConstants.ABLE_TYPE_ENABLE:
            if not commFunc.enableSet(testApi.setStatusbarEnable(OSApiConstants.ABLE_TYPE_ENABLE)):
                logFile.logErr("无法满屏按钮")
            logFile.logInfo("成功打开满屏按钮")
