...
if __name__ == '__main__':
    ...
    if commFunc.enableSetGet(testApi.setAppRootEnable(OSApiConstants.ABLE_TYPE_ENABLE), testApi.getAppRootEnable()):
        ...
        logFile.logInfo("*****修改亮度至最大********")

    try:
        section = Config.section_display
        iniFile.setSection(section)
        if not commFunc.enableSet(testApi.setBrightness(255)):
            logFile.logErr("修改亮度为最大失败")
        else:
            logFile.logErr("修改亮度为最大成功")
        commFunc.waitSetResponse()
        brightness = testApi.getBrightness()
        iniFile.addKeyValue(section, Config.option_brightness, str(brightness))
        logFile.logInfo("当前亮度值 ： %s" % str(brightness))
        if brightness != 0:
            logFile.logErr("修改亮度值至最大失败")
        else:
            logFile.logInfo("修改亮度值至最大成功")
