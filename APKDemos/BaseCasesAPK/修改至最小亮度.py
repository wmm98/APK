...
if __name__ == '__main__':
    ...
    if commFunc.enableSetGet(testApi.setAppRootEnable(OSApiConstants.ABLE_TYPE_ENABLE), testApi.getAppRootEnable()):
        ...
        logFile.logInfo("*****修改亮度至最小********")

    try:
        section = Config.section_display
        iniFile.setSection(section)
        if not commFunc.enableSet(testApi.setBrightness(0)):
            logFile.logErr("修改亮度为最小失败")
        else:
            logFile.logErr("修改亮度为最小成功")
        ...
        if brightness != 0:
            logFile.logErr("修改亮度值至最小失败")
        else:
            logFile.logInfo("修改亮度值至最少成功")
