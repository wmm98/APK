...
if __name__ == '__main__':
    ...
    logFile.logInfo("****内外部型号测试开始********")
    try:
        productModel = testApi.getProductModel()
        logFile.logInfo("外部型号为： %s" % productModel)

        internalModel = baseFunc.getInternalModel()
        logFile.logInfo("内部型号为： %s " % internalModel)
