...
if __name__ == '__main__':
    ...
    logFile.logInfo("****系统类型测试开始********")
    try:
        version = baseFunc.currentUser()
        logFile.logInfo("当前版本为： %s " % version)

        systemType = baseFunc.selinuxAuth()
        logFile.logInfo("当前系统类型为： %s " % systemType)
        logFile.logInfo("注意：Enforcing 即为Selinux")
