...
if __name__ == '__main__':
    ...
    try:
        ...
        if testApi.getWifiEnable() != OSApiConstants.ABLE_TYPE_ENABLE:
            if not commFunc.enableSet(testApi.setWifiEnable(OSApiConstants.ABLE_TYPE_ENABLE)):
                logFile.logErr("使能wifi开关失败")
            else:
                logFile.logInfo("使能wifi开关成功")
        ...
        if current_data != OSApiConstants.ABLE_TYPE_ENABLE:
            logFile.logErr("wifi开关使能失败")
        else:
            logFile.logInfo("wifi开关使能成功")
