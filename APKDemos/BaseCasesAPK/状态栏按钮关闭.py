...
if __name__ == '__main__':
    ...
    logFile.logInfo("****关闭满屏按钮********")
    section = Config.section_system_ui
    iniFile.setSection(section)
    try:
        option = Config.option_status_bar
        if testApi.getStatusbarEnable() != OSApiConstants.ABLE_TYPE_DISABLE:
            if not commFunc.enableSet(testApi.setStatusbarEnable(OSApiConstants.ABLE_TYPE_DISABLE)):
                logFile.logErr("关闭满屏按钮")
            logFile.logInfo("关闭打开满屏按钮")
            commFunc.waitSetResponse()
        status_bar = testApi.getStatusbarEnable()
        logFile.logInfo("当前的满屏按钮为：%d" % status_bar)
        if status_bar >= 0:
            iniFile.addKeyValue(section, option, str(status_bar))
        else:
            iniFile.addKeyValue(section, option, baseFunc.dealOSErrCode(status_bar))
