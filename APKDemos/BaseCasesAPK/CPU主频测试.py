...
if __name__ == '__main__':
    ...
    logFile.logInfo("****CPU主频测试开始********")
    try:
        for i in range(baseFunc.getCpuCoresNum()):
            logFile.logInfo("cpu 核%d 的 当前主频为：%s" % (i, str(baseFunc.currentCpuFrequency(i))))
            logFile.logInfo("cpu 核%d 的 最大主频为：%s" % (i, str(baseFunc.maxCpuFrequency(i))))
            logFile.logInfo("cpu 核%d 的 最小主频为：%s" % (i, str(baseFunc.minCpuFrequency(i))))
