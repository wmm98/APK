...
if __name__ == '__main__':
    ...
    if commFunc.enableSetGet(testApi.setAppRootEnable(OSApiConstants.ABLE_TYPE_ENABLE), testApi.getAppRootEnable()):
        ...
        logFile.logInfo("*****当前亮度********")

    try:
        iniFile.setSection(Config.section_display)
        current_brightness = testApi.getBrightness()
        iniFile.addKeyValue(Config.section_display,  Config.option_brightness, str(current_brightness))
        logFile.logInfo("当前亮度为%d" % current_brightness)
